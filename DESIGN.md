...
| ② | **支出** | 毎日〜月数回 | 支出の追加・月移動・絞り込み・行内編集。明細管理に集中 |
...
2. **支出は追加から修正まで一か所に集める** — 同じ月の集計見出しをホームと一覧の両方に出さない
...
| ② | 支出一覧 | 週1〜月1 | **支出として残す**。入力・検索・編集をまとめる |
...
月移動の直下に**クイック入力**を置き、「PayPay 以外は即入力」という元の設計意図を維持する。その下は精算額・共有支出合計・2人の立替額・双方の確認を1つの精算ブロックにまとめ、枠を増やさず区切り線で読む順序を示す。カテゴリ・支払い手段別の内訳と過去の清算は補足情報として後段に置く。
...
PayPay 以外の支出入力と、月ごとの支出一覧・絞り込み・行内編集・削除をまとめる。カテゴリの追加は入力欄の「+ 新規」から行うため、独立したカテゴリ管理画面は持たない。
...
- 支出 = `Expense` の CRUD ＋ `Category` の追加
