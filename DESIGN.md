...
**① ダッシュボード（＝アプリ起動画面／即入力の入口）** — 最上部が**クイック支出登録フォーム**。その下に今月の集計・カテゴリ別グラフ・支払い手段内訳・最近の支出。アプリを開いた瞬間に入力を始められるレイアウトで、「PayPay 以外は即入力」ルールを行動誘導する。

![ダッシュボード](mockups/01-dashboard.svg)

**② 支出一覧** — フィルタ付き一覧・検索・ページング。**新規登録はここではなくダッシュボードに集約**。ここは閲覧・編集・削除に特化。

![支出一覧](mockups/02-expenses.svg)

**③ 月次清算** — その月の集計・送金額の提示・双方の「確認」ボタン・カテゴリ内訳

![月次清算](mockups/03-settlement.svg)

**④ PayPay 取り込み** — CSV アップロード → 未判定行に「共有／個人」判定＆カテゴリ付与 → 一括保存

![PayPay 取り込み](mockups/04-paypay-import.svg)

**⑤ カテゴリ管理** — 追加・並び替え・インライン編集・アーカイブ。使用中カテゴリは削除不可（アーカイブで代替）

![カテゴリ管理](mockups/05-categories.svg)
...
![ログイン](mockups/06-login.svg)

**⑦ アカウントメニュー** — 右上ユーザー名から開くドロップダウン。表示名変更・パスワード変更・テーマ切替（ライト/ダーク）・「全端末からログアウト」・ログアウトを内包。前回ログイン時刻も表示。

![アカウントメニュー](mockups/07-account.svg)

**画面から §2 ドメインへの対応**：
- ダッシュボード／清算画面 = `MonthlySettlement` の集計＋状態表示
- 支出一覧 = `Expense` の CRUD、登録はダッシュボード
- PayPay 取り込み = `PayPayImportStaging` → `Expense` への昇格フロー
- カテゴリ管理 = `Category` の CRUD（`is_archived` で論理削除）
- ログイン／アカウント = `User` の認証・設定変更（Phase 5）

### 1.8 AI 連携を採用しない判断（2026-08-18 決定）

カリキュラム Phase 3 の学習項目に「AI 連携のサーバ側実装（Vertex AI / Gemini）」があり、本アプリでの用途として **PayPay 明細からのカテゴリ自動推定** を検討したが、**採用しない**と決めた。

**理由：このアプリの規模では AI が解く問題が存在しない**

| 前提 | AI の必要性 |
| :---- | :---- |
| 支出は月40件程度 | 手で分類しても負担にならない |
| カテゴリは5つ | 選択肢が少なく迷いにくい |
| 常連の店は10店舗程度 | 店舗名の部分一致ルールでほぼ当たる |
| 利用者は2人固定 | 表記ゆれや未知の店がほとんど発生しない |

一方で導入すると **外部依存・課金・レイテンシ・データ送信のリスク** が増える。§0.1 の **YAGNI（今必要なコードのみ追加するべき）** に反する。

**決め手になった制約**：カテゴリ推定に使える情報は実質 **店舗名だけ**（金額と日付では判別不能）。店舗名は「どこで何を買ったか」の行動履歴であり、パートナーの分も含まれる。成功指標（§1.5）への寄与が小さい機能のために、これを外部 API へ送る判断はしなかった。

**将来の再検討ポイント**：レシート画像の OCR は画像認識が必須なので AI でしか実現できない。§1.6 のスコープ外項目として残しており、v2 で必要になった時点で改めて検討する。

**この判断自体が Phase 3 の成果**：カリキュラム §4 の「選んだアプローチが後で『合わなかった』と気づくのも立派な学び」に沿う。学習項目として提示された機能を、自分のアプリの文脈で評価して不要と結論づけるところまでが判断。
...
`/openapi.json` の `paths` は 14 件、オペレーションは 18 本。
...
| AI 連携 | **実装しない** | カリキュラム Phase 3 の学習項目だが、本アプリでは不要と判断した（§1.8）。項目5 は「検討して見送る」という結論で完了扱い |
