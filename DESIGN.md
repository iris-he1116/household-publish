...
### 1.7 UI — 3画面構成（2026-09-15 情報設計を再整理）

現在の画面構成。既存の `mockups/` は Phase 4 着手時点のワイヤーフレームであり、以下の情報設計を決める前の参考資料として残している。

| # | 画面 | 使用頻度 | 役割 |
| :---- | :---- | :---- | :---- |
| ① | **ホーム** | 毎日 | クイック入力と、その月の集計・送金額・双方の確認 |
| ② | **支出** | 毎日〜月数回 | 月移動・絞り込み・行内編集。明細管理だけに集中 |
...
当初の7画面を Phase 4 で3画面へ絞った。2026-08-18 時点では支出入力・月次集計・支出一覧をホームへ統合したが、実データで使うと、ホームの月次集計と独立した清算画面の情報が重複し、支出管理と精算の目的も混在した。

2026-09-15 に次の原則で再整理した。

1. **ホームは「すぐ入力」と「いま誰が誰にいくら送るか」に集中する** — 日常の入力導線を維持しつつ、精算額と状態も分かる
2. **支出一覧は検索と修正に絞る** — 入力はホームに一本化し、同じ月の集計見出しも重複させない
3. **タブは3個のままにする** — PCのトップナビとスマホのボトムナビの視認性を保つ
4. **旧清算URLは維持する** — `/settlement` と `/settlement/[ym]` は年月を保ってホームへ転送する

| # | 当初の画面 | 使用頻度 | 判断 |
| :---- | :---- | :---- | :---- |
| ① | ダッシュボード | 毎日 | **ホームとして残す**。月次清算の中心画面にする |
| ② | 支出一覧 | 週1〜月1 | **支出として残す**。検索・編集に絞る |
| ③ | 月次清算 | 月1 | **ホームに統合**。同種の集計を別画面へ重複させない |
| ④ | PayPay 取り込み | 週1〜月1 | **残す**。核心機能 |
| ⑤ | カテゴリ管理 | **年に数回** | **独立画面は廃止**。入力欄の「+ 新規」と「編集」で追加・名称変更する |
| ⑥ | ログイン | Phase 5 | **Phase 5 に送る**（認証が前提のため） |
| ⑦ | アカウントメニュー | **ほぼ使わない** | **廃止**。2人固定なので表示名を変える機会がない。設定はドロップダウンに収める |

**理由**

カテゴリ管理とアカウント設定を独立画面にしない判断は維持する。カテゴリは支出入力欄の「+ 新規」で追加し、「編集」のダイアログで名称を変更する。2人固定の設定は必要になった時点でヘッダーへ収める。

**廃止した機能の行き先**

| 廃止した画面の機能 | 行き先 |
| :---- | :---- |
| 支出一覧のフィルタ・編集・削除 | 「支出」タブに集約。編集は**行内で直接**行う |
| カテゴリの追加 | 支出入力欄の「+ 新規」ボタン |
| カテゴリ名の変更 | 支出入力欄の「編集」ダイアログ。過去の支出にも反映 |
| カテゴリの並び替え・アーカイブ | **作らない**。必要になったら追加する |
| 表示名変更・パスワード変更・テーマ切替 | Phase 5 でヘッダーのドロップダウンに収める |

なお `categories` の API（4オペレーション）と `is_archived` カラムは Phase 3 で実装済みであり、**削除しない**。名称変更だけをUIへ出し、並び替え・アーカイブは必要になった時点で追加できる状態を保つ。

---

#### ① ホーム（＝月次清算ダッシュボード）

ホームの先頭に**クイック入力**を置き、「PayPay 以外は即入力」という元の設計意図を維持する。その下は月移動・対象月の見出し・精算額・共有支出合計・2人の立替額・双方の確認・カテゴリと支払い手段別の内訳までを1つの精算ブロックにまとめ、枠を増やさず区切り線で読む順序を示す。独立した「ホーム」見出しや月ナビは置かず、過去の清算だけを別の補足情報として後段に置く。
「明細を見る」は同じ月を保って支出タブへ遷移する。

既存のホーム／月次清算モックは Phase 4 時点の参考資料であり、現在の実装では両者を統合している。

**スマホ版** — 月移動を「前月／次月」と短く表示し、カードを縦積みにする。

---

#### ② 支出

月ごとの支出一覧・絞り込み・行内編集・削除だけを置く。PayPay 以外の支出入力とカテゴリ追加はホームのクイック入力に一本化し、この画面には重複して置かない。

**スマホ版** — 入力フォームと一覧を縦積みにし、横幅が必要な一覧だけ横スクロールを許可する。
...
**画面から §2 ドメインへの対応**：
- ホーム = `Expense` のクイック登録 ＋ `Category` の追加・名称変更 ＋ `MonthlySettlement` の集計と状態遷移
- 支出 = `Expense` の検索・更新・削除
...
│   ├── page.tsx                 # ホーム（月次清算ダッシュボード）
│   ├── login/
│   │   └── page.tsx             # ログイン画面（未認証時のみアクセス可）
│   ├── expenses/
│   │   └── page.tsx             # 支出の一覧・絞り込み・行内編集
│   ├── settlement/              # 旧URL互換（ホームへ転送）
│   └── paypay-import/
