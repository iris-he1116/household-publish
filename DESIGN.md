...
| # | 画面 | 使用頻度 | 役割 |
| :---- | :---- | :---- | :---- |
| ① | **ホーム** | 毎日 | その月の集計・送金額・双方の確認。月次清算の状況確認に集中 |
| ② | **支出** | 毎日〜月数回 | 支出の追加・月移動・絞り込み・行内編集 |
...
1. **ホームは「いま誰が誰にいくら送るか」に集中する** — 起動直後に精算額と状態が分かる
...
表示月を1か所で切り替え、**送金方向と金額を最上部で最大表示**する。その下に共有支出合計と2人の立替額、双方の確認、カテゴリ・支払い手段別の内訳、過去の清算を置く。
「明細を見る」は同じ月を保って支出タブへ遷移し、「支出を追加」は支出入力へ移動する。
...
**画面から §2 ドメインへの対応**：
- ホーム = `MonthlySettlement` の集計と状態遷移
